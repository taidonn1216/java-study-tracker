...
> 以下のように、**リポジトリ（DBアクセス）→ サービス（業務ロジック）→ コントローラー（画面の制御）** という3層構造になっています。
...
| ユーザー名 | 所有データ |
|---|---|
| `user` | Java科目、データベース科目 |
| `user2` | Spring Framework科目 |

> パスワードは BCrypt でハッシュ化されています。 `data.sql` を参照してください。
...
| Controller テスト | `TrackerControllerTest` | 画面表示・所有者チェック動作確認 |
| Repository テスト | `SubjectRepositoryImplTest` / `TaskRepositoryImplTest` | DBアクセスのテスト |
