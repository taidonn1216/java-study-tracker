...
[ブラウザ] ⇄ [Controller] ⇄ [Service] ⇄ [Repository] ⇄ [H2 Database]
              ...
              [Thymeleaf]  [所有者のチェック]  
              (HTML生成)   (ビジネスロジック)  
※ Model（Subject, Task, Userなど）は全層で共通利用されるデータ構造です              
```
...
| ② 所有者チェック | Service / Repository | `user_id` でフィルタリングし、他ユーザーのデータを除外 |
...
**② Controller / Service 層での所有者チェック**
認証済みでも、他ユーザーのデータはアクセスできません。
Controller は `Principal` からログインユーザーのIDを取得し、
Repository の SQL に `WHERE user_id = ?` を付加して自分のデータのみ返します。
...
リソース未発見時は `ResourceNotFoundException` をスローします。

**③ 自己ロックアウト防止**
`AdminController` でログイン中のユーザー自身の権限変更を禁止しています。
...
| Controller テスト | `TrackerControllerTest` / `AdminControllerTest` | 画面表示・所有者チェック動作確認 |
| Repository テスト | `SubjectRepositoryImplTest` / `TaskRepositoryImplTest` / `UserRepositoryImplTest` | DBアクセスのテスト |
| Model テスト | `SubjectTest` / `TaskTest` / `SubjectSummaryTest` / `TaskStatsTest`| モデルクラスのテスト |
