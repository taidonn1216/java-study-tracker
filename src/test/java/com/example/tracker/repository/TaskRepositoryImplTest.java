...
class TaskRepositoryImplTest {
    ...
    void testInsert() {
        ...
        assertEquals(LocalDate.parse("2026-03-01"), tasks.get(0).getDeadline());
        assertEquals("", tasks.get(0).getReflection());
    }

    @Test
    void testFindBySubjectIdAndUserId() {
    ...
    @Test
    void testFindBySubjectIdAndUserId_NoTasks() {
        List<Task> tasks = taskRepository.findBySubjectIdAndUserId(subjectId, userId);
    ...
    void testFindBySubjectIdAndStatusAndUserId() {
        ...
        List<Task> inProgress = taskRepository.findBySubjectIdAndStatusAndUserId(subjectId, TaskStatus.IN_PROGRESS,
                userId);
    ...
    @Test
    void testUpdateReflectionByIdAndUserId() {
        taskRepository.insert(subjectId, "更新対象", TaskStatus.DONE, LocalDate.parse("2026-03-01"), "");
        Long taskId = taskRepository.findBySubjectIdAndUserId(subjectId, userId).get(0).getId();

        int updated = taskRepository.updateReflectionByIdAndUserId(taskId, "新しい振り返り", userId);
        assertEquals(1, updated);

        Task task = taskRepository.findBySubjectIdAndUserId(subjectId, userId).get(0);
        assertEquals("新しい振り返り", task.getReflection());
    }

    @Test
    void testUpdateReflectionByIdAndUserId_OtherUser() {
        Long otherSubjectId = createOtherUserSubject();
        taskRepository.insert(otherSubjectId, "他人のタスク", TaskStatus.DONE, LocalDate.parse("2026-03-01"), "");
        Long otherTaskId = jdbcTemplate.queryForObject(
                "SELECT id FROM TASK WHERE subject_id = ?", Long.class, otherSubjectId);

        int updated = taskRepository.updateReflectionByIdAndUserId(otherTaskId, "書き換え", userId);
        assertEquals(0, updated);
    }
}